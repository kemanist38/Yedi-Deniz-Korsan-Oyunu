...
export type TalentId='volley'|'loader'|'longshot'|'firemaster'|'sails'|'tough'|'windcaller'|'mender'|'plunder'|'scavenger'|'sapper'|'bounty';
...
export const TALENT_BRANCHES=[
  {id:'gunnery',name:'Topçuluk',icon:'/assets/icon-attack-v1.webp',talents:['volley','loader','longshot','firemaster'] as TalentId[]},
  {id:'seamanship',name:'Denizcilik',icon:'/assets/officer-helmsman-v1.webp',talents:['sails','tough','windcaller','mender'] as TalentId[]},
  {id:'plunder',name:'Yağma',icon:'/assets/icon-chest-v1.webp',talents:['plunder','scavenger','sapper','bounty'] as TalentId[]},
];
export const TALENTS:Record<TalentId,{name:string;max:number;per:string;icon:string;apply:(b:Bonus,rank:number)=>void}>={
  volley:{name:'Keskin Nişan',max:5,per:'+%4 top hasarı',icon:'/assets/icon-attack-v1.webp',apply:(b,r)=>{b.damage*=1+.04*r;}},
  loader:{name:'Seri Dolum',max:5,per:'-%3 dolum süresi',icon:'/assets/ammo-grape-v1.webp',apply:(b,r)=>{b.reload*=1-.03*r;}},
  longshot:{name:'Uzak Menzil',max:4,per:'+10 menzil',icon:'/assets/officer-lookout-v1.webp',apply:(b,r)=>{b.range+=10*r;}},
  firemaster:{name:'Ateş Ustası',max:3,per:'+%25 yanma hasarı',icon:'/assets/ammo-fire-v1.webp',apply:(b,r)=>{b.burn*=1+.25*r;}},
  sails:{name:'Rüzgâr Okuyucu',max:5,per:'+%3 hız',icon:'/assets/icon-speed-v1.webp',apply:(b,r)=>{b.speed*=1+.03*r;}},
  tough:{name:'Sağlam Omurga',max:5,per:'-%3 alınan hasar',icon:'/assets/icon-shield-v1.webp',apply:(b,r)=>{b.taken*=1-.03*r;}},
  windcaller:{name:'Fırtına Bilgesi',max:3,per:'-%10 yetenek bekleme',icon:'/assets/icon-hat-v1.webp',apply:(b,r)=>{b.cooldown*=1-.1*r;}},
  mender:{name:'Usta Tamirci',max:4,per:'+%15 tamir hızı',icon:'/assets/icon-repair-v1.webp',apply:(b,r)=>{b.repair*=1+.15*r;}},
  ...
  bounty:{name:'Ödül Avcısı',max:5,per:'+%5 batırma altını',icon:'/assets/officer-gunner-v1.webp',apply:(b,r)=>{b.bounty+=.05*r;}},
};

export const OFFICERS:Record<OfficerId,{name:string;title:string;icon:string;per:string;apply:(b:Bonus,rank:number)=>void}>={
  gunner:{name:'Topçu Başı',title:'Barut ve gülle ustası',icon:'/assets/officer-gunner-v1.webp',per:'+%5 top hasarı',apply:(b,r)=>{b.damage*=1+.05*r;}},
...
export function spentPoints(crew:CrewState){return Object.values(crew.talents).reduce((s,r)=>s+(r||0),0);}
...
export function computeBonus(crew:CrewState):Bonus{
  ...
  for(const [id,rank] of Object.entries(crew.talents))if(rank)TALENTS[id as TalentId].apply(b,rank);
