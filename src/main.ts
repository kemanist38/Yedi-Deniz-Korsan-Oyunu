...
import {spawnLightning,spawnFrost,updateAbilityFx,drawAbilityFx} from './abilityFx';
...
// Yetenek animasyon denemesi: hayalet geçişte adacıklardan geçilir; gemi yarı saydam ve arkasında hayalet izi
...
function fireAtTarget(){
  ...
  const side=fx*ty-fy*tx>0?-1:1,damage=state.cannon*BALL_DAMAGE*(1+upgrades.damage*.11)*cannon.damage*bonus.damage*mix*(1+equipBonus().damage+achBonus().damage)*useConsumable('powder');
...
function effectiveSpeed(){return(128+upgrades.speed*5)*bonus.speed*(1+equipBonus().speed)*(abilityActive('speed')?SPEED_BOOST:1);}
...
function renderEliteShips(){
  ...
  ui('eliteShipDetail').innerHTML=`<span class="eyebrow">Elit ${ship.level}</span><img class="elite-art elite-preview" src="${eliteArtUrl(ship.id)}" alt="${ship.name}" draggable="false"/><h3>${ship.name}</h3><em>${ship.english}</em><dl><dt>Gemi</dt><dd>${(BASE_HP+ship.level*HP_PER_ELITE).toLocaleString('tr-TR')} temel can · ${BASE_CANNONS+ship.level*CANNONS_PER_ELITE} top yuvası</dd><dt>Özel yetenek</dt><dd>Yeniden tasarlanıyor — yakında</dd></dl><button id="equipEliteShip" ${ship.level>1&&locked||activeShip===ship.id?'disabled':''}>${activeShip===ship.id?'AKTİF GEMİ':ELITE_TEST_MODE?'TEST ET':ship.level===1&&!elitePurchased?`SATIN AL · ${ELITE_ONE_PRICE} İNCİ`:locked?'KİLİTLİ':'GEMİYİ SEÇ'}</button>`;
...
function update(dt:number){
  ...
  for(let i=shots.length-1;i>=0;i--){
    ...
    if(s.owner==='player'){
      ...
      const taken=s.damage*bonus.taken*useConsumable('shield');
...
function activateEliteAbility(){toast('Elit gemilerin özel yetenekleri yakında');}
...
let ghostFade=0;
const shipAlpha=()=>ghostFade>0?ghostFade:ghostTimer>0?.55:1;
...
if(import.meta.env.DEV)(window as any).__ky={abilityDemo:(kind:'lightning'|'freeze'|'ghost')=>{
    if(kind==='ghost'){ghostTimer=5;return;}const t=selected;if(!t)return;
    if(kind==='lightning'){spawnLightning(player,t);const hit=5000;t.hp-=hit;damageText(t.x,t.y,hit);t.aggro=true;t.combatTimer=12;}
    else{t.frozen=4;spawnFrost(t,4);}},treasure,rollTreasurePart,manual:(on:boolean)=>{manualClock=on;},step:(dt:number)=>{update(dt);draw();},noFade:()=>{mapFade=0;},cinematic,makeShip,NPCS,MONSTERS,state,player,camera,enemies,monsters,respawn,enterMap,mapDef,fleetOwner,lootChests,sparkles,sinkEnemy,shots,particles,wrecks,select:(t:Target)=>{selected=t;state.attacking=true;},route:(v:Vec)=>{routeTarget=navigablePoint(v);destination=routeVia(routeTarget);},fleetNav,get routeTarget(){return routeTarget;},get selected(){return selected;},get destination(){return destination;}};
