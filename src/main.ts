...
function renderShipMenu(){
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-storage-depot]').forEach(button=>button.onclick=()=>{selectedCannonKind=button.dataset.storageDepot as CannonKind;cannonInventory[selectedCannonKind]>0?moveCannon(selectedCannonKind,true):renderShipMenu();});
  document.querySelectorAll<HTMLButtonElement>('[data-storage-ship]').forEach(button=>button.onclick=()=>{selectedCannonKind=button.dataset.storageShip as CannonKind;mountedCannons[selectedCannonKind]>0?moveCannon(selectedCannonKind,false):renderShipMenu();});
...
function moveCannon(kind:CannonKind,toShip:boolean){
  const unlockLevel:Record<CannonKind,number>={cast:1,long:2,rapid:3,heavy:4};
  if(toShip){if(state.level<unlockLevel[kind]){toast(`Bu top için seviye ${unlockLevel[kind]} gerekli`);renderShipMenu();return;}if(cannonInventory[kind]<=0||mountedCannonCount()>=cannonCapacity()){if(mountedCannonCount()>=cannonCapacity())toast('Geminin top kapasitesi dolu');renderShipMenu();return;}cannonInventory[kind]--;mountedCannons[kind]++;}
