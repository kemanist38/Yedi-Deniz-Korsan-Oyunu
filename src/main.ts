...
type Enemy = Vec & { kind:'ship'; def?:NpcDef; boss?:BossDef; summoned?:boolean; escortsCalled?:boolean; burnTimer?:number; burnDps?:number; tower?:boolean; towerIndex?:number;  hitRadius?:number; fireRange?:number; rewardXp?:number; role:EnemyRole; angle:number; hp:number; maxHp:number; cooldown:number; speed:number; damage:number; reload:number; rewardGold:number; rewardFame:number; color:string; name:string; tier:number; aggro:boolean; wander:number; slowTimer:number; homeX:number; homeY:number; combatTimer:number };
...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  <main class="game-shell">
    ...
    <section class="hud">
      ...
      <div class="portal-prompt" id="portalPrompt"></div><div class="quest-overlay ach-overlay" id="achOverlay"><section class="daily-window ach-window"><header><div><span class="eyebrow">Madalyalar ve kalıcı bonuslar</span><h2>BAŞARIMLAR</h2></div><button id="closeAch" aria-label="Kapat">×</button></header><p class="ach-summary" id="achSummary"></p><div class="ach-grid" id="achGrid"></div></section></div><div class="quest-overlay daily-overlay" id="dailyOverlay"><section class="daily-window"><header><div><span class="eyebrow">Her gün gel, ödül büyüsün</span><h2>GÜNLÜK ÖDÜL</h2></div><button id="closeDaily" aria-label="Kapat">×</button></header><div class="daily-grid" id="dailyGrid"></div><p class="daily-note" id="dailyNote"></p></section></div><button class="dig-prompt" id="digPrompt"><img src="/assets/icon-treasure-map-v1.webp" alt="" draggable="false"/><span>HAZİNEYİ KAZ</span><i><em id="digBar"></em></i></button><div class="event-panel" id="eventPanel"></div><div class="quest-overlay world-overlay" id="worldMapOverlay"><section class="world-scroll" aria-label="Dünya haritası"><button class="scroll-close" id="closeWorldMap" aria-label="Dünya haritasını kapat">×</button><div class="world-chart" id="worldChart"></div><div id="worldInfo" hidden></div></section></div><div class="quest-overlay" id="questOverlay"><section class="quest-log"><header><div><span class="eyebrow">Kaptanın görev defteri</span><h2>DENİZ GÖREVLERİ</h2></div><button id="closeQuests" aria-label="Görevleri kapat">×</button></header><p class="quest-intro">Aynı anda yalnızca bir görev yürütülebilir. İptal edilen veya tamamlanan görev 8 saat sonra yeniden açılır.</p><div class="quest-list" id="questList"></div></section></div>
...
const effectiveMaxHp=()=>Math.round((state.maxHp+(eliteEnabled()?eliteShip().level*HP_PER_ELITE:0))*eliteMaxHpFactor()*(1+equipBonus().hp+achBonus().hp));
...
function fireAtTarget(){
  ...
  const side=fx*ty-fy*tx>0?-1:1,damage=state.cannon*BALL_DAMAGE*(1+upgrades.damage*.11)*cannon.damage*bonus.damage*eliteDamageFactor(selected)*eliteCritFactor()*mix*(1+equipBonus().damage+achBonus().damage)*useConsumable('powder');
...
function openAchievements(){renderAchievements();ui('achOverlay').classList.add('open');}
function closeAchievements(){saveAchievements(ach);ui('achOverlay').classList.remove('open');}
