...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  <main class="game-shell">
    ...
    <section class="hud">
      <div class="corner-buttons"><button class="round-button" id="openShop" aria-label="Market"><img src="/assets/icon-market-v1.webp?v=3d-20260926" alt="" draggable="false"/></button><button class="round-button" id="openMenu" aria-label="Menü"><img src="/assets/icon-menu-v1.webp?v=3d-20260926" alt="" draggable="false"/></button></div>
      <nav class="shop-tabs" id="shopTabs"><button id="openEliteShips" data-shop="eliteShipOverlay"><img class="nav-img" id="shipNavIcon" alt="" draggable="false"/><span>TERSANE</span></button><button id="openEquipShop" data-shop="equipShopOverlay"><i class="nav-img equip-nav"></i><span>DONANIM</span></button><button id="openCannonShop" data-shop="cannonShopOverlay"><img class="nav-img" src="/assets/gunner-vignette-v1.webp?v=3d-20260926" alt="" draggable="false"/><span>TOPLAR</span></button><button id="openShip" data-shop="shipOverlay"><img class="nav-img" src="/assets/icon-inventory-v2.webp?v=3d-20260926" alt="" draggable="false"/><span>ENVANTER</span></button><button id="openMarket" data-shop="marketOverlay"><img class="nav-img" src="/assets/ammo-fire-v1.webp?v=3d-20260926" alt="" draggable="false"/><span>GÜLLELER</span></button><button id="openLoadoutTab" data-shop="supplyOverlay"><img class="nav-img" src="/assets/icon-chest-v1.webp?v=3d-20260926" alt="" draggable="false"/><span>MALZEMELER</span></button><button class="shop-close" id="closeShopTabs" aria-label="Marketi kapat">×</button></nav>
      <div class="captain-overlay menu-overlay" id="menuOverlay"><section class="captain-profile menu-window"><header><div><span class="eyebrow">Kaptan köşkü</span><h2>MENÜ</h2></div><button id="closeMenu" aria-label="Menüyü kapat">×</button></header><div class="menu-grid"><button id="openCaptain"><img src="/assets/captain-bust-v1.webp?v=3d-20260926" alt="" draggable="false"/><span>KAPTAN PROFİLİ</span></button><button id="openGuild"><i class="nav-img guild-nav"></i><span>FİLO</span></button><button id="openDevelopment"><img src="/assets/icon-anvil-v1.webp?v=3d-20260926" alt="" draggable="false"/><span>GELİŞTİRME</span></button><button id="openCrew"><img src="/assets/officer-helmsman-v1.webp?v=3d-20260926" alt="" draggable="false"/><span>TAYFA</span></button><button id="openDaily"><img src="/assets/icon-chest-v1.webp?v=3d-20260926" alt="" draggable="false"/><span>GÜNLÜK ÖDÜL</span></button><button id="openMenuQuests"><img src="/assets/icon-scroll-v1.webp?v=3d-20260926" alt="" draggable="false"/><span>GÖREVLER</span></button><button id="openMenuWorld"><img id="menuWorldIcon" alt="" draggable="false"/><span>DÜNYA HARİTASI</span></button><button id="openSettings"><img src="/assets/icon-gear-v1.webp?v=3d-20260926" alt="" draggable="false"/><span>AYARLAR</span></button></div></section></div>
...
const playerShipImage=new Image();playerShipImage.src='/assets/starter-ship-v2.webp?v=3d-20260926';
...
const directionalShipImage=new Image();directionalShipImage.src='/assets/starter-ship-dir-v2.webp?v=3d-20260926';
document.documentElement.style.setProperty('--pirate-icons','url("/assets/pirate-ui-icons-v1.webp?v=3d-20260926")');
([['/assets/icon-world-v1.webp?v=3d-20260926','worldMapIcon'],['/assets/icon-world-v1.webp?v=3d-20260926','menuWorldIcon'],['/assets/icon-ship-nav-v1.webp?v=3d-20260926','shipNavIcon']] as const).forEach(([src,id])=>{const img=document.getElementById(id) as HTMLImageElement|null;if(img)img.src=src;});
const cannonAssetSources:Record<CannonKind,string>={cast:'/assets/cannon-cast-v1.webp?v=3d-20260926',long:'/assets/cannon-long-v1.webp?v=3d-20260926',rapid:'/assets/cannon-rapid-v1.webp?v=3d-20260926',heavy:'/assets/cannon-heavy-v1.webp?v=3d-20260926'};
const rasterItemAssets:Partial<Record<QuickItemId,string>>={};
(Object.keys(SPECIAL_AMMO) as SpecialAmmo[]).forEach(k=>{rasterItemAssets[k]=SPECIAL_AMMO[k].icon;});rasterItemAssets.mine=ABILITIES.mine.icon;rasterItemAssets.shield=CONSUMABLES.shield.icon;rasterItemAssets.powder=CONSUMABLES.powder.icon;rasterItemAssets.speed=ABILITIES.speed.icon;rasterItemAssets.repairkit='/assets/icon-repair-v1.webp?v=3d-20260926';
rasterItemAssets.iron='/assets/ammo-iron-v1.webp?v=3d-20260926';rasterItemAssets.chain='/assets/ammo-chain-v1.webp?v=3d-20260926';
...
const DEV_BRANCHES:{name:string;icon:string;nodes:({kind:'pearl';id:UpgradeKind}|{kind:'point';id:TalentId})[]}[]=[
  {name:'Topçuluk',icon:'/assets/icon-attack-v1.webp?v=3d-20260926',nodes:[{kind:'pearl',id:'damage'},{kind:'pearl',id:'range'},{kind:'pearl',id:'reload'},{kind:'point',id:'firemaster'},{kind:'point',id:'sapper'}]},
  {name:'Denizcilik',icon:'/assets/officer-helmsman-v1.webp?v=3d-20260926',nodes:[{kind:'pearl',id:'hull'},{kind:'pearl',id:'speed'},{kind:'pearl',id:'repair'},{kind:'point',id:'tough'},{kind:'point',id:'windcaller'}]},
  {name:'Yağma',icon:'/assets/icon-chest-v1.webp?v=3d-20260926',nodes:[{kind:'point',id:'plunder'},{kind:'point',id:'scavenger'},{kind:'point',id:'bounty'}]},
];
...
function renderWorldMap(){
  ...
  chart.innerHTML=`<div class="world-grid">${GRID.flat().map(key=>{const m=MAPS[key],locked=state.level<m.tier,here=key===currentMap,owned=fleetOwner(key)==='player',t=THEMES[m.tier];
    ...
    return`<button class="world-tile ${here?'here':''} ${locked?'locked':''} ${m.safe?'safe':''} ${chartSelection===key?'selected':''}" data-chart="${key}" style="--tile:${t.sea[0]};--tile2:${t.sea[1]};--tint:${t.tint}"><span class="wt-tag">${tag}</span>${here?'<img class="wt-flag" src="/assets/icon-flag-v1.webp?v=3d-20260926" alt="Buradasın" draggable="false"/>':''}<b>${key}</b><strong>${m.name}</strong></button>`;}).join('')}</div>`;
...
function drawFleetIsland(){
  ...
  if(th==='lava'){
    ...
    const g=ctx.createRadialGradient(s.x,s.y-70,10,s.x,s.y-30,215);g.addColorStop(0,'rgba(255,222,172,.28)');g.addColorStop(.5,'rgba(255,126,112,.13)');g.addColorStop(1,'rgba(255,126,112,0)');ctx.fillStyle=g;ctx.beginPath();ctx.ellipse(s.x,s.y-30,220,105,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='rgba(255,184,145,.38)';ctx.lineWidth=5;for(const dx of [-140,-78,92,148]){ctx.beginPath();ctx.moveTo(s.x+dx,s.y-105);ctx.quadraticCurveTo(s.x+dx-20,s.y-150,s.x+dx+6,s.y-178);ctx.moveTo(s.x+dx-8,s.y-135);ctx.lineTo(s.x+dx-28,s.y-158);ctx.stroke();}
  }else if(th==='misty'){
    for(let i=0;i<3;i++){const drift=Math.sin(now*.22+i*2.1)*32;const g=ctx.createRadialGradient(s.x+drift,s.y-75+i*38,8,s.x+drift,s.y-75+i*38,245);g.addColorStop(0,'rgba(225,239,232,.13)');g.addColorStop(1,'rgba(210,232,224,0)');ctx.fillStyle=g;ctx.beginPath();ctx.ellipse(s.x+drift,s.y-75+i*38,245,58,0,0,Math.PI*2);ctx.fill();}
  }else if(th==='crimson'){
    const pulse=.7+Math.sin(now*2.2)*.15,g=ctx.createRadialGradient(s.x,s.y-90,15,s.x,s.y-40,210);g.addColorStop(0,`rgba(220,42,54,${.24*pulse})`);g.addColorStop(1,'rgba(95,12,25,0)');ctx.fillStyle=g;ctx.beginPath();ctx.ellipse(s.x,s.y-45,220,105,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='rgba(232,77,76,.3)';ctx.lineWidth=3;for(let i=0;i<4;i++){ctx.beginPath();ctx.moveTo(s.x-150+i*100,s.y-165);ctx.lineTo(s.x-130+i*92,s.y-90);ctx.stroke();}
  }else if(th==='toxic'){
    const pulse=.75+Math.sin(now*2.7)*.12,g=ctx.createRadialGradient(s.x,s.y-65,15,s.x,s.y-25,220);g.addColorStop(0,`rgba(145,232,74,${.24*pulse})`);g.addColorStop(1,'rgba(63,116,38,0)');ctx.fillStyle=g;ctx.beginPath();ctx.ellipse(s.x,s.y-25,225,110,0,0,Math.PI*2);ctx.fill();for(let i=0;i<8;i++){const a=i*.78+now*.08,r=120+(i%3)*30;ctx.fillStyle='rgba(188,239,106,.22)';ctx.beginPath();ctx.arc(s.x+Math.cos(a)*r,s.y-45+Math.sin(a)*r*.32,3+(i%2)*2,0,Math.PI*2);ctx.fill();}
  }else if(th==='storm'){
    const pulse=.6+Math.sin(now*4.5)*.18,g=ctx.createRadialGradient(s.x,s.y-100,10,s.x,s.y-55,225);g.addColorStop(0,`rgba(185,215,255,${.24*pulse})`);g.addColorStop(1,'rgba(70,100,150,0)');ctx.fillStyle=g;ctx.beginPath();ctx.ellipse(s.x,s.y-50,230,110,0,0,Math.PI*2);ctx.fill();if(Math.sin(now*3.4)>.82){ctx.strokeStyle='rgba(220,238,255,.62)';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(s.x-30,s.y-220);ctx.lineTo(s.x+2,s.y-150);ctx.lineTo(s.x-18,s.y-118);ctx.lineTo(s.x+28,s.y-62);ctx.stroke();}
  }else if(th==='verdant'){
    const g=ctx.createRadialGradient(s.x,s.y-55,10,s.x,s.y-15,215);g.addColorStop(0,'rgba(244,215,132,.18)');g.addColorStop(.55,'rgba(76,155,82,.1)');g.addColorStop(1,'rgba(76,155,82,0)');ctx.fillStyle=g;ctx.beginPath();ctx.ellipse(s.x,s.y-15,220,100,0,0,Math.PI*2);ctx.fill();
  }
  ctx.restore();
