...
function renderGuild(){
  ...
  const islandHtml=islands.length?islands.map(k=>{const m=MAPS[k],slots=islandSlots(g,k),cost=towerCost(m.tier),built=slots.filter(Boolean).length;
    return`<article class="guild-island"><header><div><span class="eyebrow">${m.key} · Seviye ${m.tier}</span><h4>${m.fleet.name}</h4></div><b>${built} / ${TOWER_SLOTS} kule</b></header><div class="tower-slots">${slots.map((t,i)=>t?`<div class="tower-slot built"><i class="tower-art"></i><small>Kule ${i+1}</small><em><span style="width:${Math.round(t.hp/t.maxHp*100)}%"></span></em></div>`:`<button class="tower-slot empty" data-build="${k}:${i}" ${g.treasury<cost?'disabled':''}><i class="tower-art ghost"></i><small>Kaide ${i+1}</small><b>DİK · ${cost} İnci</b></button>`).join('')}</div></article>`;}).join('')
...
function drawFleetIsland(){
  ...
  if(owned){for(const tw of ownTowers){const p=worldToScreen(tw);drawFleetTower(ctx,th,'player',p.x,p.y);}
    const built=new Set(ownTowers.map(t=>t.slot));ctx.textAlign='center';ctx.font='700 8px Inter';ctx.fillStyle='#e8dcb8cc';ctx.shadowColor='#000';ctx.shadowBlur=4;FLEET.towers.forEach(([dx,dy],i)=>{if(!built.has(i))ctx.fillText('BOŞ KAİDE',s.x+dx,s.y+dy+4);});ctx.shadowBlur=0;}
