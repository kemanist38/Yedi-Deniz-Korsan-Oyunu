...
function defeatBoss(e:Enemy){
  const b=e.boss!;stopBossMusic();state.fame+=b.xp;state.pearls+=b.pearls;bossOf(currentMap).pending=false;saveBosses();saveAccount();
...
function sinkEnemy(e:Enemy){
  ...
  rewardNotice(`+${goldGain} Altın   +${e.rewardFame} TP`);toast(`${e.name} batırıldı`);if(e.def)recordQuestProgress('npc',e.def.id);countBossKill(e);if(!e.summoned)setTimeout(spawnEnemy,1800);
}
...
function claimToday(){
  const st=dailyStatus(daily);if(!st.canClaim)return;const r=dailyReward(st.day,state.level);daily=claimDaily(daily);saveDaily(daily);
...
function updateTreasure(dt:number){
  ...
  const spot=treasure.active!,m=MAPS[spot.map],r=treasureReward(m.tier,NPCS[m.npcs[0]].gold);dig=null;treasure.active=null;treasure.found++;saveTreasure(treasure);
